...
sf_ami_race_plot_ordered <- sf_ami_race_wide %>%
    pivot_longer(
        cols = c(White, Black, Asian, Latinx),
        names_to = "Race",
        values_to = "Income"
    ) %>%
    mutate(Race = reorder(Race, Income))

# Let's repeat this but now for the 9 county bay area.
bay_area_median_income <- get_acs(
    geography = "county",
    variables = "B19013_001",  # Median household income
    year = 2022,
...
# This code processes median income data for the Bay Area
bay_area_ami <- bay_area_median_income %>%  # Pipe operator for chaining operations
    group_by(GEOID) %>%  # Group data by county using GEOID
    mutate(
        # Calculate low income threshold as 80% of median income estimate
        low_income = estimate * .8,
...
# You can also "filter" rows based on a condition
bay_area_ami %>% filter(GEOID == "06075")

# You can also filter text
bay_area_ami %>% filter(NAME == "San Francisco County, California")

# If you want to filter by a key word you can use "grepl"
bay_area_ami %>% filter(grepl("San", NAME))
# Not that the last three operations did not create an object in memory.
# If we wanted an object for later use we would have to assign it to a variable.
sf_ami <- bay_area_ami %>% filter(grepl("San", NAME))

# And to view the contents of this object we just call the name
sf_ami

# Rent
# Rent burden
# 1. rent burden from the census
# 2. how much do you need to make to avoid rent-burden?



# The code:
# 1. Takes a dataset 'bay_area_median_income' containing income estimates
# 2. Uses pipe operator %>% for chaining operations
# 3. Groups the data by GEOID (typically representing counties)
# 4. Creates two new columns using mutate():
#    - low_income: 80% of the median income
#    - high_income: 120% of the median income
# This appears to be setting income thresholds for analyzing income distribution
# Low income households are those that are at risk of displacement.



# Now let's look at the data
head(sf_median_income)
# Head loads the first 6 rows of the data by default. You can adjust the
# number.
head(sf_median_income, 12)


# A little something about coding:
# In your code, you want to have very clean and readable code so that it can be
# reproducible.
#
# Clean code is essential because:
# 1. It makes your work reproducible by others
# 2. It helps you understand your own code weeks/months later
# 3. It reduces errors and makes debugging easier
# 4. It facilitates collaboration with other researchers
#
# Best practices include:
# - Keep lines of code short (80 characters or less)
# - Using clear variable names
# - Adding comments to explain complex operations
# - Organizing code into logical sections
# - Consistent formatting and indentation
# - Regular documentation of data sources and transformations

#
# Here's an example of good vs. bad code indentation:
#
# Good indentation:
if (condition) {
    do_something()  # R uses snake_case by convention
    for (i in 1:10) {
        print(i)
    }
}
...
# R indentation best practices:
# 1. Use 2-4 spaces for indentation (not tabs)
# 2. Put spaces around operators and after commas: for (i in 1:10)
# 3. Put spaces inside parentheses: if (condition)
# 4. Place opening curly brace on same line as control statement
# 5. Align closing curly braces with their opening control statement
# 6. Use snake_case for function and variable names
# 7. Add spaces after comments: # Like this
#
# Good indentation makes code:
# - Easier to read
# - Easier to spot nested structures
# - Easier to find bugs
# - More maintainable
